...
<script language = "javascript">
	function checkpwd0(pwd){ //0-高严格
		...
		function checkpwd3(pwd){ //3-极严格，只满足高严格，不满足低，中严格
		  var allValid = true;
		  for (i = 0;i<pwd.length;i++)
		  {
		    ch = pwd.charAt(i);
		    if(((ch<='9' && ch >= '0')||(ch <='z' && ch >= 'a') ||(ch <='Z' && ch >='A'))&&!checkpwd2(pwd) &&!checkpwd1(pwd) ){
		    }//等价:if(checkpwd0&&!checkpwd2(pwd) &&!checkpwd1(pwd))
		    else{
		     allValid = false;
		    }
		  }
